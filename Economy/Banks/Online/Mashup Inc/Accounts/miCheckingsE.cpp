...
MASH_b miCheckingsE::RegCheckingsMI::hasAccount = false;
MASH_f miCheckingsE::RegCheckingsMI::Amount = 0.00;
MASH_int64t miCheckingsE::RegCheckingsMI::accNum = 0;
MASH_b miCheckingsE::RegCheckingsMI::hasCard = false;
MASH_b miCheckingsE::RegCheckingsMI::hasCoupon = false;

void miCheckingsE::RegCheckingsMI::Create()
{
	Universal u;
	...
	if (!hasAccount) {
		if (AppD::Age < 9) {
			...
			miCheckingsD::RegCheckingsMI checkingsD;
			miCheckingsTF::RegCheckingsMI checkingsTF;

			accNum = accNumrnd;

			miCheckingsD::RegCheckingsMI::Amount = Amount;
			miCheckingsD::RegCheckingsMI::accNum = accNum;

			miCheckingsTF::RegCheckingsMI::accNum = accNum;
			...
			if (Parent == "Father" || Parent == "father") {
				if (fatherHelp == 1) {
					...
					if (getCard != 5) {
						...
						hasCard = true;
						miCheckingsTF::RegCheckingsMI::Amount = Amount;
...
void miCheckingsE::RegCheckingsMI::Withdrawal(MASH_f num)
{
	Amount -= num;

	//Own
	miCheckingsD::RegCheckingsMI::File = AppD::Folder + "/Banks/Online/Mashup Inc/Accounts/Checking.json";

	if (MASH_fs::exists(miCheckingsD::RegCheckingsMI::File)) {
		std::ifstream owninput(miCheckingsD::RegCheckingsMI::File);
		owninput >> miCheckingsD::RegCheckingsMI::Data;
		owninput.close();

		miCheckingsD::RegCheckingsMI::Data["Regular"]["Bank Amount"] = Amount;

		std::ofstream ownoutput(miCheckingsD::RegCheckingsMI::File);
		ownoutput << std::setw(4) << miCheckingsD::RegCheckingsMI::Data << MASH_endl;
		ownoutput.close();
	...
	for (MASH_uint i = 0; i < miBankD_O::Data.size(); i++) {
		if (miBankD_O::Data[i].contains("Checking")) {
			miBankD_O::Counter = i;
	...
	miBankD_O::Data[miBankD_O::Counter]["Checking"]["Regular"]["Bank Amount"] = Amount;
	...
	BanksD::Data[BanksD::Counter]["Online"][BanksD_O::Counter]["Mashup Inc"][miBankD_O::Counter]["Checking"]["Regular"]["Bank Amount"] = Amount;
...
void miCheckingsE::RegCheckingsMI::Deposit(MASH_f num)
{
	Amount += num;
...
MASH_f miCheckingsE::RegCheckingsMI::GetBankAmount()
{
	return Amount;
}
