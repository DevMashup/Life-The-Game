...
MASH_b sfCheckingsE::RegCheckingsSF::hasAccount = false;
MASH_f sfCheckingsE::RegCheckingsSF::Amount = 0.00;
MASH_int64t sfCheckingsE::RegCheckingsSF::accNum = 0;
MASH_b sfCheckingsE::RegCheckingsSF::hasCard = false;

void sfCheckingsE::RegCheckingsSF::Create()
{
	Universal u;
	...
	if (!hasAccount) {
		if (AppD::Age < 9) {
			...
			sfCheckingsD::RegCheckingsSF checkingsD;
			sfCheckingsTF::RegCheckingsSF checkingsTF;

			accNum = accNumrnd;

			sfCheckingsD::RegCheckingsSF::Amount = Amount;
			sfCheckingsD::RegCheckingsSF::accNum = accNum;

			sfCheckingsTF::RegCheckingsSF::accNum = accNum;
			...
			if (Parent == "Father" || Parent == "father") {
				if (fatherHelp == 1) {
					...
					if (getCard != 5) {
						...
						hasCard = true;
						sfCheckingsTF::RegCheckingsSF::Amount = Amount;
...
void sfCheckingsE::RegCheckingsSF::Withdrawal(MASH_f num)
{
	Amount -= num;

	//Own
	sfCheckingsD::RegCheckingsSF::File = AppD::Folder + "/Banks/Online/Sally's Finances/Accounts/Checking.json";

	if (MASH_fs::exists(sfCheckingsD::RegCheckingsSF::File)) {
		std::ifstream owninput(sfCheckingsD::RegCheckingsSF::File);
		owninput >> sfCheckingsD::RegCheckingsSF::Data;
		owninput.close();

		sfCheckingsD::RegCheckingsSF::Data["Regular"]["Bank Amount"] = Amount;

		std::ofstream ownoutput(sfCheckingsD::RegCheckingsSF::File);
		ownoutput << std::setw(4) << sfCheckingsD::RegCheckingsSF::Data << MASH_endl;
		ownoutput.close();
	...
	for (MASH_uint i = 0; i < sfBankD_O::Data.size(); i++) {
		if (sfBankD_O::Data[i].contains("Checking")) {
			sfBankD_O::Counter = i;
	...
	sfBankD_O::Data[sfBankD_O::Counter]["Checking"]["Regular"]["Bank Amount"] = Amount;
	...
	BanksD::Data[BanksD::Counter]["Online"][BanksD_O::Counter]["Sally's Finances"][sfBankD_O::Counter]["Checking"]["Regular"]["Bank Amount"] = Amount;
...
void sfCheckingsE::RegCheckingsSF::Deposit(MASH_f num)
{
	Amount += num;
...
MASH_f sfCheckingsE::RegCheckingsSF::GetBankAmount()
{
	return Amount;
}
