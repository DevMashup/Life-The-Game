...
MASH_b phSavingsE::RegSavingsPH::hasAccount = false;
MASH_f phSavingsE::RegSavingsPH::Amount = 0;
MASH_int64t phSavingsE::RegSavingsPH::accNum = 0;
MASH_b phSavingsE::RegSavingsPH::hasCard =  false;

void phSavingsE::RegSavingsPH::Create()
{
	Universal u;
	...
	if (!hasAccount) {
		if (AppD::Age < 9) {
			...
			phSavingsD::RegSavingsPH SavingsD;
			phSavingsTF::RegSavingsPH SavingsTF;

			accNum = accNumrnd;

			phSavingsD::RegSavingsPH::Amount = Amount;
			phSavingsD::RegSavingsPH::accNum = accNum;

			phSavingsTF::RegSavingsPH::accNum = accNum;
...
void phSavingsE::RegSavingsPH::Withdrawal(MASH_f num)
{
	Amount -= num;

	phSavingsD::RegSavingsPH::File = AppD::Folder + "/Banks/In Person/United States/Missouri/Kansas City/Plaza Horizon/Accounts/Saving.json";

	if (MASH_fs::exists(phSavingsD::RegSavingsPH::File)) {
		std::ifstream owninput(phSavingsD::RegSavingsPH::File);
		owninput >> phSavingsD::RegSavingsPH::Data;
		owninput.close();

		phSavingsD::RegSavingsPH::Data["Regular"]["Bank Amount"] = Amount;

		std::ofstream ownoutput(phSavingsD::RegSavingsPH::File);
		ownoutput << std::setw(4) << phSavingsD::RegSavingsPH::Data << MASH_endl;
		ownoutput.close();
	}
	else {
...
void phSavingsE::RegSavingsPH::Deposit(MASH_f num)
{
	Amount -= num;
...
MASH_f phSavingsE::RegSavingsPH::GetBankAmount()
{
	return Amount;
}
