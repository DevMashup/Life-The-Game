...
MASH_b spbCheckingsE::RegCheckingsSPB::hasAccount = false;
MASH_f spbCheckingsE::RegCheckingsSPB::Amount = 0.00;
MASH_int64t spbCheckingsE::RegCheckingsSPB::accNum = 0;
MASH_b spbCheckingsE::RegCheckingsSPB::hasCard = false;

void spbCheckingsE::RegCheckingsSPB::Create()
{
	Universal u;
	...
	if (!hasAccount) {
		if (AppD::Age < 9) {
			...
			MASH_cout << "Thank you for choosing Summit Prairie Bank. Unfourtanly, since your only " << AppD::Age;
			...
			spbCheckingsD::RegCheckingsSPB checkingsD;
			spbCheckingsTF::RegCheckingsSPB checkingsTF;

			accNum = accNumrnd;

			spbCheckingsD::RegCheckingsSPB::Amount = Amount;
			spbCheckingsD::RegCheckingsSPB::accNum = accNum;

			spbCheckingsTF::RegCheckingsSPB::accNum = accNum;
			...
			if (Parent == "Father" || Parent == "father") {
				if (fatherHelp == 1) {
					...
					if (getCard != 5) {
						...
						MASH_cout << "As a thankyou for opening an account at Summit Prairie Bank, you get an initial deposit of 250 " << AppD::Currency << "." << MASH_endl;
						Deposit(250.00);

						hasCard = true;
						spbCheckingsTF::RegCheckingsSPB::Amount = Amount;
						...
						MASH_cout << " when you're 16 or older and purchase you're own credit card but as a thankyou for opening an account at Summit Prairie Bank, you get an initial deposit of 250 " << AppD::Currency << "." << MASH_endl;
						Deposit(250.00);
			...
			checkingsD.CreateData();
			MASH_cout << "Thank you for opening an account at Summit Prairie Bank As a reward, you'll get an intial deposit of 250 " << AppD::Currency << "." << MASH_endl;
			Deposit(250.00);
			u.SV(2);
...
void spbCheckingsE::RegCheckingsSPB::Withdrawal(MASH_f num)
{
	Amount -= num;

	spbCheckingsD::RegCheckingsSPB::File = AppD::Folder + "/Banks/In Person/United States/Missouri/Kansas City/Summit Prairie Bank/Accounts/Checking.json";

	if (MASH_fs::exists(spbCheckingsD::RegCheckingsSPB::File)) {
		std::ifstream owninput(spbCheckingsD::RegCheckingsSPB::File);
		owninput >> spbCheckingsD::RegCheckingsSPB::Data;
		owninput.close();

		spbCheckingsD::RegCheckingsSPB::Data["Regular"]["Bank Amount"] = Amount;

		std::ofstream ownoutput(spbCheckingsD::RegCheckingsSPB::File);
		ownoutput << std::setw(4) << spbCheckingsD::RegCheckingsSPB::Data << MASH_endl;
		ownoutput.close();
	}
	else {
	...
	BanksD::Data[BanksD::Counter]["In Person"][BanksD_I::Counter]["United States"][usBanksD_I::Counter]["Missouri"]
		[moBanksD_I::Counter]["Kansas City"][kcmoBanksD_I::Counter]["Summit Prairie Bank"][spbBankD_I::Counter]
...
void spbCheckingsE::RegCheckingsSPB::Deposit(MASH_f num)
{
	Amount += num;
...
MASH_f spbCheckingsE::RegCheckingsSPB::GetBankAmount()
{
	return Amount;
}
