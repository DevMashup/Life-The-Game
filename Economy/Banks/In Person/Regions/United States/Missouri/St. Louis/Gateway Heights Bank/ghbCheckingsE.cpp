...
MASH_b ghbCheckingsE::RegCheckingsGHB::hasAccount = false;
MASH_f ghbCheckingsE::RegCheckingsGHB::Amount = 0.00;
MASH_int64t ghbCheckingsE::RegCheckingsGHB::accNum = 0;
MASH_b ghbCheckingsE::RegCheckingsGHB::hasCard = false;

void ghbCheckingsE::RegCheckingsGHB::Create()
{
	Universal u;
	...
	if (!hasAccount) {
		if (AppD::Age < 9) {
			...
			MASH_cout << "Thank you for choosing Gateway Heights Banks. Unfourtanly, since your only " << AppD::Age;
			...
			ghbCheckingsD::RegCheckingsGHB checkingsD;
			ghbCheckingsTF::RegCheckingsGHB checkingsTF;

			accNum = accNumrnd;

			ghbCheckingsD::RegCheckingsGHB::Amount = Amount;
			ghbCheckingsD::RegCheckingsGHB::accNum = accNum;

			ghbCheckingsTF::RegCheckingsGHB::accNum = accNum;
			...
			if (Parent == "Father" || Parent == "father") {
				if (fatherHelp == 1) {
					...
					if (getCard != 5) {
						...
						hasCard = true;
						ghbCheckingsTF::RegCheckingsGHB::Amount = Amount;
...
void ghbCheckingsE::RegCheckingsGHB::Withdrawal(MASH_f num)
{
	Amount += num;

	//Own
	std::ifstream owninput(ghbCheckingsD::RegCheckingsGHB::File);
	owninput >> ghbCheckingsD::RegCheckingsGHB::Data;
	owninput.close();

	ghbCheckingsD::RegCheckingsGHB::Data["Regular"]["Bank Amount"] = Amount;

	std::ofstream ownoutput(ghbCheckingsD::RegCheckingsGHB::File);
	ownoutput << std::setw(4) << ghbCheckingsD::RegCheckingsGHB::Data << MASH_endl;
	ownoutput.close();

	//Bank
	std::ifstream bankinput(ghbBankD_I::File);
	bankinput >> ghbBankD_I::Data;
	bankinput.close();

	for (MASH_uint i = 0; i < ghbBankD_I::Data.size(); i++) {
		if (ghbBankD_I::Data[i].contains("Checking")) {
	...
	ghbBankD_I::Data[ghbBankD_I::Counter]["Checking"]["Regular"]["Bank Amount"] = Amount;

	std::ofstream bankoutput(ghbBankD_I::File);
	bankoutput << std::setw(4) << ghbBankD_I::Data << MASH_endl;
	bankoutput.close();
	...
	BanksD::Data[BanksD::Counter]["In Person"][BanksD_I::Counter]["United States"][usBanksD_I::Counter]["Missouri"]
		[moBanksD_I::Counter]["St. Louis"][slmoBanksD_I::Counter]["Gateway Heights Bank"][ghbBankD_I::Counter]
...
void ghbCheckingsE::RegCheckingsGHB::Deposit(MASH_f num)
{
	Amount += num;
...
MASH_f ghbCheckingsE::RegCheckingsGHB::GetBankAmount()
{
	return Amount;
}
