...
MASH_b ghbSavingsE::RegSavingsGHB::hasAccount = false;
MASH_f ghbSavingsE::RegSavingsGHB::Amount = 0.00;
MASH_int64t ghbSavingsE::RegSavingsGHB::accNum = 0;
MASH_b ghbSavingsE::RegSavingsGHB::hasCard = false;

void ghbSavingsE::RegSavingsGHB::Create()
{
	Universal u;
	...
	if (!hasAccount) {
		if (AppD::Age < 9) {
			...
			ghbSavingsD::RegSavingsGHB SavingsD;
			ghbSavingsTF::RegSavingsGHB SavingsTF;

			accNum = accNumrnd;

			ghbSavingsD::RegSavingsGHB::Amount = Amount;
			ghbSavingsD::RegSavingsGHB::accNum = accNum;

			ghbSavingsTF::RegSavingsGHB::accNum = accNum;
			...
			if (Parent == "Father" || Parent == "father") {
				if (fatherHelp == 1) {
					...
					if (getCard != 5) {
						...
						hasCard = true;
						ghbSavingsTF::RegSavingsGHB::Amount = Amount;
...
void ghbSavingsE::RegSavingsGHB::Withdrawal(MASH_f num)
{
	Amount -= num;

	//Own
	std::ifstream owninput(ghbSavingsD::RegSavingsGHB::File);
	owninput >> ghbSavingsD::RegSavingsGHB::Data;
	owninput.close();

	ghbSavingsD::RegSavingsGHB::Data["Regular"]["Bank Amount"] = Amount;

	std::ofstream ownoutput(ghbSavingsD::RegSavingsGHB::File);
	ownoutput << std::setw(4) << ghbSavingsD::RegSavingsGHB::Data << MASH_endl;
	ownoutput.close();
	...
	for (MASH_uint i = 0; i < ghbBankD_I::Data.size(); i++) {
		if (ghbBankD_I::Data[i].contains("Saving")) {
	...
	ghbBankD_I::Data[ghbBankD_I::Counter]["Saving"]["Regular"]["Bank Amount"] = Amount;
...
void ghbSavingsE::RegSavingsGHB::Deposit(MASH_f num)
{
	Amount += num;
...
MASH_f ghbSavingsE::RegSavingsGHB::GetBankAmount()
{
	return Amount;
}
