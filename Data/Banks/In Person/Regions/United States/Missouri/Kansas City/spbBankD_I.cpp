...
MASH_str spbBankD_I::Folder = " ";
MASH_str spbBankD_I::File = " ";
MASH_b spbBankD_I::isData = false;
MASH_json spbBankD_I::Data;
MASH_uint spbBankD_I::Counter = 0;

void spbBankD_I::CreateData()
{
	CreateFolder();
	CreateFile();
}

void spbBankD_I::CreateFolder() 
{
	Universal u;

	Folder = kcmoBanksD_I::Folder + "/Summit Prairie Bank";
...
void spbBankD_I::UpdateCityFile()
{
}

void spbBankD_I::UpdateBanksFile()
{
	...
	if (!isData) {
		MASH_json newData = {
			{"Summit Prairie Bank", MASH_json::array()}
		};
	...
	for (MASH_uint i = 0; i < element.size(); i++) {
		if (element[i].contains("Summit Prairie Bank"))
		{
